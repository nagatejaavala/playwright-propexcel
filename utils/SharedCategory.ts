 ...
 * Vendor list (confirmed): Maintenance, Cleaning, IT Services, Utilities,
 * Office Supplies, Consulting, Legal, Marketing, Security, Transport, Other
 * Note: "Emergency" exists on request but NOT on vendor — do not include it.
 */
 ...
 * Override with: CATEGORY=Consulting npx playwright test ...
 */
  ...
  if (fs.existsSync(DATA_FILE)) {
    try {
      ...
      if (typeof data.index === 'number' && Number.isFinite(data.index)) {
        // Prefer matching by category name if list order changed
