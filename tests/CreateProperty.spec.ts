...
// Use the `browser` fixture supplied by Playwright Test, not a manual top-level launch.
...
test('Propexcel Automation', async ({ browser }) => {
    ...
    if (await leadsSearch.count() > 0) {
        ...
        if (await searchFallback.count() > 0) {
            ...
            await searchFallback.fill('ganesh');
    ...
    if (await leadName.count() > 0) {
        ...
        await leadName.fill('ganesh');
    } else {
        const nameFallback = page.getByLabel(/name|full name/i).first();
        if (await nameFallback.count() > 0) await nameFallback.fill('ganesh').catch(() => { });
    }
    await page.waitForTimeout(2000); // Wait for 5 seconds

    await page.locator('input[type="email"]').fill('ganesh@propexcel.com');
    await page.waitForTimeout(2000); // Wait for 5 seconds
    ...
    let convertedDealName = 'ganesh Deal';
    ...
    if (await dealRow.count() > 0) {
        ...
        if (await dealsSearchInput.count() > 0) {
            await fillIfVisible(dealsSearchInput, 'ganesh');
            await dealsSearchInput.press('Enter').catch(() => { });
            await page.waitForTimeout(3000);

            const searchResult = page.locator("xpath=(//div[contains(normalize-space(.), 'ganesh')])[1]").first();
            if (await searchResult.count() > 0) {
                await clickIfVisible(searchResult);
            } else {
                const textResult = page.getByText(/ganesh/i).first();
    ...
    await page.locator('//input[@placeholder="Search..."]').fill('ganesh');
...
// Search
await page.getByPlaceholder('Search...').fill('sathish');

// Wait for result
await page.waitForSelector('text="sathish (Tenant)"');

// Select
await page.getByText('sathish (Tenant)', { exact: true }).click();
    ...
    await expect(receivePaymentButton).toBeVisible({ timeout: 90000 });
...
const accountDropdown = page.locator(
    "div.flex-1.truncate",
).filter({
    hasText: "Select account..."
});
...
// =====================================
// Step 2: Search for Account "4000"
// =====================================

const accountSearch = page.locator("input[name='reference_no']").first();
...
// =====================================
// Step 3: Select "4000 - Rental Income (Revenue / Operating Revenue)"
// =====================================

const accountOption = page.getByText(
    "4000 - Rental Income (Revenue / Operating Revenue)",
    { exact: true }
);
...
await accountOption.click();
    ...
    await expect(savePaymentButton).toBeVisible({ timeout: 90000 });
