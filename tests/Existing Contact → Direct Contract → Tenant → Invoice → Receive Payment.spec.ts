/**
 * Existing Contact → Direct Contract → Tenant → Invoice → Receive Payment
 * Login (org.json) → Accounts → Contracts → Create New Contract
 * → select existing contact (crm-contacts-leads.json contacts[]) → vacant property
 * → edit PROPERTY RENTAL (rent / discount / tax) → Save → Submit for Approval
 * → approve contract → tenant user → move-in → invoice → Razorpay payment.
 *
 * Skips Lead and Deal entirely (direct contract from Accounts).
 * Needs: org.json + crm-contacts-leads.json (run Creating Contacts,Leads,Deals first).
 * Saves: tenant-new-org.json
 *
 * Run:
 *   npx playwright test "tests/Existing Contact → Direct Contract → Tenant → Invoice → Receive Payment.spec.ts" --headed
 */
...
import { ensureGst18Tax } from '../utils/TaxSettings';
...
/**
 * After Submit for Approval on a direct contract: confirm dialog + Internal Approval if shown.
 */
async function approveDirectContractWorkflow(page: import('@playwright/test').Page) {
  await dismissEndToEndFlowTour(page);
  await dismissNotificationsModal(page);

  if (await page.getByText(/^Active$/i).first().isVisible({ timeout: 3000 }).catch(() => false)) {
    console.log('Contract already Active — skipping approval workflow');
    return;
  }

  const confirmDialog = page.getByRole('dialog').filter({
    hasText: /Submit for Approval|Confirm|Are you sure/i,
  }).first();
  ...
  if (await approveContractBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
    console.log('Approve Contract already available — skipping workflow Approve');
    return;
  }

  const workflowSection = page.getByText(/Approval Workflow/i).first();
  if (!(await workflowSection.isVisible({ timeout: 8000 }).catch(() => false))) {
    console.log('No Approval Workflow section — will use Approve Contract');
    return;
  }
  ...
  if (await approveBtn.isVisible({ timeout: 15000 }).catch(() => false)) {
    ...
    console.log('Clicked Approve on contract approval workflow');
    ...
    if (await confirmApprove.isVisible({ timeout: 8000 }).catch(() => false)) {
      ...
      console.log('Clicked Confirm on contract approval step');
    }
    await page.getByText(/APPROVED|COMPLETED ON|Active|Approve Contract/i)
      .first()
...
test('Existing Contact → Direct Contract → Tenant → Invoice → Receive Payment', async ({ page, context }) => {
  const propertyNamePlaceholder = `villa${Date.now().toString().slice(-6)}`;
  const data = { fullName: '', email: '', mobile: '', propertyName: propertyNamePlaceholder };
  ...
  // Prefer last contact so Existing Contact / New Deal (random pick) are less likely to collide
  const sharedContact = crm.contacts[crm.contacts.length - 1];
  data.fullName = sharedContact.fullName;
  data.email = sharedContact.email;
  data.mobile = sharedContact.mobile;
  console.log('Run data (direct contract from SharedCrmData contact):', data, 'Move-in date:', moveInDate);
  console.log('Direct Contract admin login (from CreateOrganization org.json):', {
    orgId: admin.orgId,
    email: admin.email,
    orgName: admin.orgName,
  });
  console.log('Picked shared CRM contact (last unused preference):', sharedContact);
...

  {
    ...
    await dismissEndToEndFlowTour(page);
    await dismissNotificationsModal(page);
    await ensureGst18Tax(page);
    await configureRazorpayIntegration(page);
  }

  {
    // Accounts → Contracts → Create New Contract
    try {
      ...
      await page.goto('https://test.propexcel.com/accounts/contracts', { waitUntil: 'domcontentloaded' });
    }
    await page.getByRole('heading', { name: /^Contracts$/i }).first().waitFor({ timeout: 30000 });
    await dismissEndToEndFlowTour(page);
    await dismissNotificationsModal(page);

    const createBtn = page.getByRole('button', { name: /Create New Contract|\+?\s*Create New Contract/i }).first();
    await createBtn.waitFor({ state: 'visible', timeout: 15000 });
    await createBtn.click();
    await page.waitForURL(/\/accounts\/contracts\/(?!create)/, { timeout: 45000 }).catch(() => undefined);
    await page.getByRole('tab', { name: /Contract Summary/i }).waitFor({ timeout: 30000 });
    console.log('Opened Create New Contract (direct)');
  }

  {
    // + Add Tenant → Contacts tab → select shared contact → Save
    const addTenantBtn = page.getByRole('button', { name: /\+?\s*Add Tenant/i }).first();
    await addTenantBtn.waitFor({ state: 'visible', timeout: 15000 });
    await addTenantBtn.click();

    const contactsDialog = page.getByRole('dialog').filter({ hasText: /Contacts|Tenants/i }).first();
    await contactsDialog.waitFor({ state: 'visible', timeout: 15000 });

    const contactsTab = contactsDialog.getByRole('tab', { name: /^Contacts$/i })
      .or(contactsDialog.getByRole('button', { name: /^Contacts$/i }))
      .or(contactsDialog.getByText(/^Contacts$/i));
    if (await contactsTab.first().isVisible({ timeout: 5000 }).catch(() => false)) {
      await contactsTab.first().click();
    }

    const searchBox = contactsDialog.getByPlaceholder(/^Search$/i)
      .or(contactsDialog.getByRole('textbox', { name: /Search|Contact/i }))
      .first();
    if (await searchBox.isVisible({ timeout: 5000 }).catch(() => false)) {
      await searchBox.fill(sharedContact.fullName);
      await page.waitForTimeout(800);
    }

    const escapedName = sharedContact.fullName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const contactCard = contactsDialog.getByRole('button', { name: new RegExp(`${escapedName}.*Email:`, 'i') }).first();
    await contactCard.waitFor({ state: 'visible', timeout: 15000 });
    await contactCard.click();
    console.log('Selected contact for direct contract:', sharedContact.fullName);

    const saveContactBtn = contactsDialog.getByRole('button', { name: /^Save$/i });
    await expect(saveContactBtn).toBeEnabled({ timeout: 15000 });
    await saveContactBtn.click();
    await contactsDialog.waitFor({ state: 'hidden', timeout: 30000 });
    await page.getByText(sharedContact.fullName, { exact: false }).first().waitFor({ state: 'visible', timeout: 15000 });
  }

  {
    // + Add Property → Vacant → random vacant → Add Property
    const addPropertyBtn = page.getByRole('button', { name: /\+?\s*Add Property/i }).first();
    await addPropertyBtn.waitFor({ state: 'visible', timeout: 15000 });
    await addPropertyBtn.click();

    const addPropertyDialog = page.getByRole('dialog').filter({ hasText: /Add Properties to Contract/i }).first();
    ...
    const vacantProperty = vacantCards.nth(index);
    data.propertyName = ((await vacantProperty.textContent()) || '').trim() || data.propertyName;
    console.log(`Selected vacant property for contract: ${data.propertyName} [${index + 1}/${count}]`);
    ...
    await addPropertyDialog.waitFor({ state: 'hidden', timeout: 30000 });
  }

  {
    // PROPERTY RENTAL table row → pencil edit → rent / discount / tax → Save
    const rentalRow = page.getByRole('row')
      .filter({ has: page.getByRole('cell', { name: /^Property Rental$/i }) })
      .filter({ hasText: /INR/i })
      .first();
    await rentalRow.waitFor({ state: 'visible', timeout: 20000 });
    await rentalRow.scrollIntoViewIfNeeded();

    // Actions: view, edit (pencil), delete — click edit (2nd button)
    const editPencil = rentalRow.getByRole('button').nth(1);
    await editPencil.click();
    console.log('Opened Property Rental edit form');

    await page.getByText(/Rental Value/i).first().waitFor({ state: 'visible', timeout: 15000 });

    const rent = Math.floor(100000 + Math.random() * (600000 - 100000 + 1));
    const discount = Math.floor(5 + Math.random() * (15 - 5 + 1));

    const rentField = page.getByLabel(/Rental Value/i)
      .or(page.getByRole('spinbutton', { name: /Rental Value/i }))
      .or(page.locator('label:has-text("Rental Value")').locator('xpath=following::input[1]'))
      .first();
    await rentField.waitFor({ state: 'visible', timeout: 10000 });
    await rentField.click();
    await rentField.fill('');
    await rentField.fill(String(rent));
    console.log('Set rental value:', rent);

    const discountByLabel = page.getByLabel(/Discount\s*\(%\)/i)
      .or(page.locator('label:has-text("Discount (%)") + * input, label:has-text("Discount (%)") ~ input').first())
      .or(page.getByRole('textbox', { name: /Discount/i }))
      .or(page.getByRole('spinbutton', { name: /Discount/i }));
    await discountByLabel.first().waitFor({ state: 'visible', timeout: 10000 });
    await discountByLabel.first().click();
    await discountByLabel.first().fill('');
    await discountByLabel.first().fill(String(discount));
    console.log('Set discount %:', discount);

    const taxCombo = page.getByRole('combobox', { name: /^Tax$/i })
      .or(page.getByLabel(/^Tax$/i))
      .or(page.getByRole('combobox').filter({ hasText: /No Tax|GST|Tax/i }))
      .first();
    await taxCombo.click();
    const gstOption = page.getByRole('option', { name: /GST\s*\(\s*18\s*%\s*\)|GST.*18/i }).first();
    if (await gstOption.isVisible({ timeout: 5000 }).catch(() => false)) {
      await gstOption.click();
      console.log('Selected tax: GST (18%)');
    } else {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (label && !/^No Tax$/i.test(label)) {
          await options.nth(i).click();
          console.log('Selected tax:', label);
          picked = true;
          break;
        }
      }
      if (!picked) {
        ...
        console.log('Selected first tax option');
      }
    }
    ...
    await saveBtn.click();
    await page.getByText(/Rental Value/i).first().waitFor({ state: 'hidden', timeout: 15000 }).catch(() => undefined);
    console.log('Saved property rental details');
  }

  {
    // Save Contract (optional) → Submit for Approval or Approve Contract (no workflow orgs)
    const saveContractBtn = page.getByRole('button', { name: /^Save Contract$/i }).first();
    if (await saveContractBtn.isVisible({ timeout: 5000 }).catch(() => false)
      && await saveContractBtn.isEnabled().catch(() => false)) {
    ...
    const approveContractBtn = page.getByRole('button', { name: /^Approve Contract$/i }).first();

    if (await submitBtn.isVisible({ timeout: 5000 }).catch(() => false)
      && await submitBtn.isEnabled().catch(() => false)) {
      await submitBtn.click();
      console.log('Clicked Submit for Approval');
      await approveDirectContractWorkflow(page);
      await approveContractUntilActive(page);
    } else if (await approveContractBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await approveContractBtn.click();
      console.log('Clicked Approve Contract (no submit workflow)');
      const confirmDialog = page.getByRole('dialog').first();
      if (await confirmDialog.isVisible({ timeout: 4000 }).catch(() => false)) {
        const confirm = confirmDialog.getByRole('button', {
          name: /Approve|Confirm|Yes|Submit/i,
        }).last();
        if (await confirm.isVisible({ timeout: 3000 }).catch(() => false)) {
          await confirm.click();
        }
      ...
      // Direct approve: wait for Active badge next to contract number (skip shared helper race)
      const activeBadge = page.locator('main').getByText(/^Active$/i).first();
      await activeBadge.waitFor({ state: 'visible', timeout: 90000 });
      console.log('Contract Active after direct Approve Contract');
    } else {
      // Already approved on a prior attempt
      const alreadyActive = page.locator('main').getByText(/^Active$/i).first();
      if (await alreadyActive.isVisible({ timeout: 5000 }).catch(() => false)) {
        console.log('Contract already Active');
      } else {
        throw new Error('Neither Submit for Approval nor Approve Contract button found');
      }
    }
  }
