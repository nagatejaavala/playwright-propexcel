/**
 * Existing Contact → Deal → Contract → Tenant → Invoice → Receive Payment
 * Login (org.json) → CRM Deals → Create Deal (existing contact from crm-contacts-leads.json, skip lead)
 ...
 *
 * Run:
 *   npx playwright test "tests/Existing Contact → Deal → Contract → Tenant → Invoice → Receive Payment.spec.ts" --headed
 */
...
test('Existing Contact → Deal → Contract → Tenant → Invoice → Receive Payment', async ({ page, context }) => {
