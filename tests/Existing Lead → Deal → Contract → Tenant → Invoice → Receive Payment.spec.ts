/**
 * Existing Lead → Deal → Contract → Tenant → Invoice → Receive Payment
 * Login (org.json) → CRM Leads → open unconverted lead (crm-contacts-leads.json.unconvertedLead)
 * → Convert to Deal → vacant property → approve
 * → contract → tenant → invoice → Razorpay payment.
 * Needs: org.json + crm-contacts-leads.json with unconvertedLead
 *   (run Creating Contacts,Leads,Deals first — creates 1 lead without Convert to Deal).
 * Saves: tenant-new-org.json
 *
 * Run:
 *   npx playwright test "tests/Existing Lead → Deal → Contract → Tenant → Invoice → Receive Payment.spec.ts" --headed
 */
...
test('Existing Lead → Deal → Contract → Tenant → Invoice → Receive Payment', async ({ page, context }) => {
  ...
  if (!crm.unconvertedLead) {
    throw new Error(
      'No unconvertedLead in SharedCrmData — run Creating Contacts,Leads,Deals.spec.ts first ' +
        '(creates 1 lead without Convert to Deal).',
    );
  }
  const sharedLead = crm.unconvertedLead;
  data.fullName = sharedLead.fullName;
  data.email = sharedLead.email;
  data.mobile = sharedLead.mobile;
  console.log('Run data (unconverted lead from SharedCrmData):', data, 'Move-in date:', moveInDate);
  console.log('Existing Lead→Payment admin login (from CreateOrganization org.json):', {
    orgId: admin.orgId,
    email: admin.email,
    orgName: admin.orgName,
  });
  console.log('Picked shared CRM unconverted lead:', sharedLead);
  ...
  await context.grantPermissions(['geolocation'], { origin: 'https://test.propexcel.com' });
      ...
      {
          // CRM → Leads → open unconverted lead → Convert to Deal (required)
          const escapedName = sharedLead.fullName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
          ...
          await page.getByRole('heading', { name: /Leads/i }).first().waitFor({ timeout: 30000 });
          await dismissEndToEndFlowTour(page);
          await dismissNotificationsModal(page);

          const leadSearch = page.getByPlaceholder(/Search/i).first()
            .or(page.getByRole('combobox', { name: /Search/i }).first());
          if (await leadSearch.isVisible({ timeout: 5000 }).catch(() => false)) {
            await leadSearch.fill(sharedLead.fullName);
          ...
          await dismissEndToEndFlowTour(page);
          await dismissNotificationsModal(page);
          console.log('Opened existing lead for:', sharedLead.fullName);
          ...
          console.log('Converted existing lead to deal:', sharedLead.fullName);

          // Land on Deal Details (convert may navigate, otherwise open from Deals list)
          const onDealDetails = await page.getByRole('heading', { name: 'Deal Details', level: 1 })
          ...
          if (!onDealDetails) {
            ...
            if (await dealSearch.isVisible({ timeout: 5000 }).catch(() => false)) {
              await dealSearch.fill(sharedLead.fullName);
              await page.waitForTimeout(1500);
            }
          ...
          console.log('On Deal Details from existing lead:', sharedLead.fullName);
      }
      {
