/**
 * New Contact → Deal → Contract → Tenant → Invoice → Payment
 * Login (org.json) → Create Contact → CRM Deals → Create Deal (skip lead)
 * → vacant property → approve → contract → tenant → invoice → Razorpay payment.
 *
 * Needs: org.json + Flow1-NewOrganization run once (Razorpay, GST, Deal Approve workflow).
 * Saves: tenant-new-org.json
 *
 * Run:
 *   npx playwright test "tests/New Contact → Deal → Contract → Tenant → Invoice → Payment.spec.ts" --headed
 */
...
test('New Contact → Deal → Contract → Tenant → Invoice → Payment', async ({ page, context }) => {
  ...
  console.log('Selected new contact for deal:', data.fullName);
