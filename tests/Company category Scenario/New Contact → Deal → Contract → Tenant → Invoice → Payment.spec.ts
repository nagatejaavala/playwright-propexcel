/**
 * Company category — New Contact → Deal → Contract → Tenant → Invoice → Payment
 * Same as Individual flow, but Contact Type = Company with random company name/reg no.
...
test('Company Category — New Contact → Deal → Contract → Tenant → Invoice → Payment', async ({ page, context }) => {
  ...
  console.log('New contact created:', data.fullName, data.email, data.mobile);

  // CRM → Deals → Create Deal (skip lead)
  // Search by person fullName; row may show company name. Deals list cards use fullName.
  ...
  await selectContactInCreateDealDialog(page, dealDialog, data.fullName, {
    matchAlso: [data.companyName, data.email].filter(Boolean) as string[],
  });
  console.log('Selected new contact for deal:', data.fullName, data.companyName);
