/**
 * Company category — New Deal → … Payment (company contacts from crm-contacts-leads-company.json)
 */
...
import { selectContactInCreateDealDialog, submitCreateDealDialog } from "../../utils/CreateDealDialog";
...
/** Accounts → Taxes → ensure GST 18% exists (shared helper). */
...
test('Company Category — New Deal → Contract → Tenant → Invoice → Receive Payment', async ({ page, context }) => {
  ...
  data.companyName = sharedContact.companyName;
  ...
  await context.grantPermissions(['geolocation'], { origin: 'https://test.propexcel.com' });
      ...
      }
      {
          ...
          // Search by person fullName; row may show company name
          ...
          await selectContactInCreateDealDialog(page, dealDialog, sharedContact.fullName, {
            matchAlso: [sharedContact.companyName, sharedContact.email].filter(Boolean) as string[],
          });
          console.log('Selected existing contact for deal:', sharedContact.fullName, sharedContact.companyName);
          ...
          await submitCreateDealDialog(page, dealDialog);
