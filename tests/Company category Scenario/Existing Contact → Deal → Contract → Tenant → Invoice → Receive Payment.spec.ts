/**
 * Company category — Existing Contact → Deal → … Payment
 * Uses company contacts from crm-contacts-leads-company.json
 */
...
test('Company Category — Existing Contact → Deal → Contract → Tenant → Invoice → Receive Payment', async ({ page, context }) => {
  ...
  await context.grantPermissions(['geolocation'], { origin: 'https://test.propexcel.com' });
      ...
      {
          // CRM → Deals → Create Deal with existing shared contact (skip lead)
